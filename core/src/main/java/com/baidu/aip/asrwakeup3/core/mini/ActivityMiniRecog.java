...
public class ActivityMiniRecog extends AppCompatActivity implements EventListener {
    ...
    protected TextView temResult;
    protected TextView bestResult;
...
//            "需要测试离线命令词识别功能可以将本类中的enableOffline改成true，首次测试离线命令词请联网使用。之后请说出“打电话给李四”";
     ...
     */
    private void stop() {
        printLog("停止语音识别：ASR_STOP");
        Toast.makeText(getApplicationContext(), "停止语音识别：ASR_STOP", Toast.LENGTH_SHORT).show();
        temResult.setText("每次启动语音识别需点击开始按钮。");
        txtResult.setText("");
        bestResult.setText("");
        ...
        if (name.equals(SpeechConstant.CALLBACK_EVENT_ASR_PARTIAL)) {
            ...
            if (params.contains("\"nlu_result\"")) {
                ...
                try {
                    ...
                    temResult.setText("临时识别结果:"+jsonObject.getJSONArray("results_recognition").getString(0));
                ...
                try {
                    jsonObject=new JSONObject(params);
                    txtResult.setText("最终识别结果:"+jsonObject.getJSONArray("results_recognition").getString(0));
                    bestResult.setText("最佳识别结果:"+jsonObject.getString("best_result"));
                } catch (JSONException e) {
                    e.printStackTrace();
                }
    ...
    private void initView() {
        temResult = (TextView) findViewById(R.id.temResult);
        temResult.setText("每次启动语音识别需点击开始按钮。");
        txtResult = (TextView) findViewById(R.id.txtResult);
        bestResult = (TextView) findViewById(R.id.bestResult);
